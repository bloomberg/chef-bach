...
ruby_block "initialize-ssh-keys" do
    block do
        ...
        if get_config('ssl-certificate').nil? then
            temp = %x[openssl req -config /tmp/openssl.cnf -extensions v3_req -new -x509 -passout pass:temp_passwd -newkey rsa:4096 -out /dev/stdout -keyout /dev/stdout -days 1095 -subj "/C=US/ST=NY/L=New York/O=Bloomberg/OU=#{node[:bcpc][:region_name]}/CN=#{node[:bcpc][:domain_name]}/emailAddress=#{node[:bcpc][:admin_email]}"]
