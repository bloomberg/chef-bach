...
class ClusterAssignRoles
  ...
  def install_basic(target_nodes)
    target_nodes.each do |node|
      install_stub(node: node) unless ridley.node.find(node[:fqdn])
  ...
  #
  #
  # install_stub uses knife bootstrap to install and configure
  # chef-client + chef-vault on a new node.
  #
  # It takes a cluster.txt entry and an optional runlist as arguments.
  #
  # Calling install_stub on an already-installed node will upgrade chef
  # and re-set its runlist, but is otherwise harmless.
  #
  def install_stub(node:, runlist: 'recipe[bcpc::chef_vault_install]')
  ...
  def car_cli
    ...
    unless %w{basic bootstrap hadoop kafka}.include?(install_type)
      raise "Install type must be one of basic, bootstrap, hadoop, kafka. " \
